...
const heroValueModules = [
  {
    title: "咨询承接",
    desc: "回答学生关于分数线、专业、就业、校园生活等高频问题。",
  },
  {
    title: "生源识别",
    desc: "提取学生画像、关注点与报考意向，筛选值得人工跟进的学生。",
  },
  {
    title: "策略反哺",
    desc: "汇总真实咨询数据，辅助高校优化宣传内容与招生策略。",
  },
];
...
function HeroValueCard({ title, desc, index }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 16 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.32 + index * 0.08, duration: 0.58, ease: [0.22, 1, 0.36, 1] }}
      className="rounded-[1.35rem] border border-white/75 bg-white/50 p-4 text-left shadow-[0_16px_46px_rgba(14,165,233,0.08)] backdrop-blur-xl"
    >
      <div className="mb-3 h-px w-10 bg-sky-400/80" />
      <h3 className="text-[15px] font-black tracking-[-0.03em] text-slate-950">{title}</h3>
      <p className="mt-2 text-[13px] font-light leading-6 text-slate-500">{desc}</p>
    </motion.div>
  );
}

function ConsultationPreviewCard() {
  return (
    <motion.div
      initial={{ opacity: 0, x: 24, y: 12, rotate: 1.2 }}
      animate={{ opacity: 1, x: 0, y: 0, rotate: 0 }}
      transition={{ delay: 0.34, duration: 0.74, ease: [0.22, 1, 0.36, 1] }}
      className="relative z-20 w-full rounded-[2rem] border border-white/80 bg-white/72 p-5 shadow-[0_26px_90px_rgba(15,23,42,0.10)] backdrop-blur-2xl"
    >
      <div className="mb-4 flex items-start justify-between gap-4 border-b border-slate-100 pb-3.5">
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.18em] text-sky-500">Student Consultation</p>
          <h3 className="mt-1 text-lg font-black tracking-[-0.04em] text-slate-950">学生咨询入口</h3>
        </div>
        <div className="text-right text-[11px] font-semibold leading-5 text-slate-400">
          <p>官网 / 公众号</p>
          <p>小程序 / 宣讲现场</p>
        </div>
      </div>
      <div className="space-y-3">
        <div className="max-w-[86%] rounded-[18px] rounded-bl-md border border-slate-200/80 bg-white px-4 py-3 text-[13px] leading-6 text-slate-600 shadow-sm">
          老师，我的分数大概能冲哪些专业？更想了解就业和保研情况。
        </div>
        <div className="ml-auto max-w-[88%] rounded-[18px] rounded-br-md bg-sky-500 px-4 py-3 text-[13px] leading-6 text-white shadow-[0_14px_34px_rgba(14,165,233,0.22)]">
          可以先参考近三年录取位次，再结合你关注的就业、保研与课程方向做专业对比。
        </div>
        <div className="max-w-[84%] rounded-[18px] rounded-bl-md border border-slate-200/80 bg-white px-4 py-3 text-[13px] leading-6 text-slate-600 shadow-sm">
          我比较关注电子信息类，也想知道毕业后主要去哪些行业。
        </div>
      </div>
      <div className="mt-4 rounded-2xl border border-sky-100 bg-sky-50/70 px-4 py-3">
        <p className="text-[11px] font-black uppercase tracking-[0.18em] text-sky-500">已形成线索标签</p>
        <p className="mt-1 text-[13px] font-semibold leading-6 text-slate-600">广东考生 ｜ 电子信息 ｜ 就业关注 ｜ 高意向</p>
      </div>
    </motion.div>
  );
}

function AdmissionsInsightPreviewCard() {
  return (
    <motion.div
      initial={{ opacity: 0, x: -18, y: 26, rotate: -1.3 }}
      animate={{ opacity: 1, x: 0, y: 0, rotate: 0 }}
      transition={{ delay: 0.52, duration: 0.78, ease: [0.22, 1, 0.36, 1] }}
      className="relative z-30 ml-auto mt-[-2.7rem] w-[92%] rounded-[1.85rem] border border-white/80 bg-white/78 p-5 shadow-[0_30px_96px_rgba(14,165,233,0.14)] backdrop-blur-2xl"
    >
      <div className="mb-4 flex items-start justify-between gap-5">
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">Admissions Office</p>
          <h3 className="mt-1 text-lg font-black tracking-[-0.04em] text-slate-950">招生办数据洞察</h3>
        </div>
        <div className="rounded-xl border border-slate-100 bg-white/70 px-3 py-2 text-right shadow-sm">
          <p className="text-[11px] font-semibold text-slate-400">近 7 天</p>
          <p className="text-base font-black text-sky-600">128 条</p>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-2.5">
        {[
          { value: "41", label: "待跟进学生" },
          { value: "+32%", label: "就业话题增长" },
          { value: "18", label: "高频问题更新" },
        ].map((item) => (
          <div key={item.label} className="rounded-2xl border border-slate-100 bg-white/62 px-3 py-2.5 text-center shadow-sm">
            <div className="text-lg font-black tracking-[-0.04em] text-slate-950">{item.value}</div>
            <div className="mt-1 text-[11px] font-medium leading-4 text-slate-400">{item.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 space-y-3">
        {insightTopics.map((topic) => (
          <div key={topic.name}>
            <div className="mb-1 flex items-center justify-between text-[12px] font-bold text-slate-500">
            ...
            <div className="h-2 overflow-hidden rounded-full bg-slate-100">
              <motion.div
                ...
                transition={{ delay: 0.78, duration: 0.68, ease: [0.22, 1, 0.36, 1] }}
      ...
      <div className="mt-4 rounded-2xl border border-slate-100 bg-white/62 px-4 py-3">
        <p className="text-[12px] font-black text-slate-700">跟进建议</p>
        <p className="mt-1 text-[12px] leading-5 text-slate-500">优先联系关注电子信息类且多次追问就业去向的高意向学生，并补充专业就业案例内容。</p>
      </div>
    </motion.div>
  );
}
...
function HeroProductPreview() {
  return (
    <div className="relative mx-auto w-full max-w-[34rem] lg:ml-auto">
      <div className="absolute -left-10 top-10 h-72 w-72 rounded-full bg-sky-200/28 blur-3xl" />
      <div className="absolute -right-8 bottom-10 h-72 w-72 rounded-full bg-cyan-200/24 blur-3xl" />
      <motion.div
        initial={{ opacity: 0, scaleY: 0.2 }}
        animate={{ opacity: 1, scaleY: 1 }}
        transition={{ delay: 0.64, duration: 0.68, ease: [0.22, 1, 0.36, 1] }}
        className="absolute left-[14%] top-[44%] z-10 hidden h-32 w-px origin-top bg-gradient-to-b from-sky-300/0 via-sky-300/80 to-sky-300/0 lg:block"
      />
      <motion.div
        initial={{ opacity: 0, y: 8 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.72, duration: 0.48 }}
        className="absolute left-[18%] top-[50%] z-40 hidden rounded-xl border border-sky-100 bg-white/80 px-3 py-2 text-[11px] font-bold tracking-[0.08em] text-sky-600 shadow-[0_12px_30px_rgba(14,165,233,0.13)] backdrop-blur-md lg:block"
      >
        咨询内容结构化沉淀
      </motion.div>
      <ConsultationPreviewCard />
      <AdmissionsInsightPreviewCard />
    </div>
  );
}
...
function HeroPage() {
  return (
    <section className="relative min-h-[100svh] overflow-hidden bg-[#fbfdff] text-slate-950">
      ...
      <div className="relative z-10 flex min-h-[100svh] items-center px-6 py-12 sm:px-8 lg:py-16">
        <div className="mx-auto grid w-full max-w-7xl items-center gap-12 lg:grid-cols-[1.02fr_0.98fr] xl:gap-16">
          <div className="text-left">
            <motion.div
              ...
              className="mb-9"
            >
              <p className="text-2xl font-black tracking-[-0.05em] text-slate-950 sm:text-3xl">招生智脑</p>
              <p className="mt-2 text-sm font-semibold tracking-[0.18em] text-sky-600">高校数字招生解决方案</p>
            </motion.div>

            <motion.h1
              initial={{ opacity: 0, y: 26 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.08, duration: 0.86, ease: [0.22, 1, 0.36, 1] }}
              className="max-w-3xl text-4xl font-black leading-[1.08] tracking-[-0.065em] text-slate-950 sm:text-6xl lg:text-[4.35rem]"
            >
              把线上咨询，转化为高校可跟进的生源线索
            </motion.h1>
            ...
            <motion.p
              ...
              className="mt-6 max-w-2xl text-base font-light leading-8 text-slate-500 sm:text-lg"
            >
              招生智脑为高校搭建专属智能招生助手，部署于官网、公众号、小程序与线下宣讲场景，承接学生咨询、识别高意向生源，并为招生办生成数据洞察与跟进建议。
            </motion.p>

            <div className="mt-8 grid max-w-3xl gap-3 sm:grid-cols-3">
              {heroValueModules.map((item, index) => (
                <HeroValueCard key={item.title} {...item} index={index} />
              ))}
            </div>
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.62, duration: 0.62, ease: [0.22, 1, 0.36, 1] }}
              className="mt-8 flex items-center gap-4"
            >
              <span className="h-px w-12 bg-sky-400/70" />
              <p className="text-base font-black tracking-[-0.03em] text-slate-800 sm:text-lg">从被动答疑，到主动招生。</p>
            </motion.div>
          </div>
...
const leadModules = [
  {
    title: "信息沉淀",
    desc: "在学生咨询过程中记录省份、分数、关注专业、咨询问题与表达倾向。",
  },
  {
    title: "意向识别",
    desc: "根据提问频次、关注深度与犹豫点，判断学生的报考意愿和跟进优先级。",
  },
  {
    title: "人工跟进",
    desc: "为招生老师生成线索摘要、沟通重点与推荐材料，提高后续转化效率。",
  },
];

const leadWorkflow = ["咨询沉淀", "标签提取", "意向评分", "人工跟进"];
...
const leadStudents = [
  ...
  },
  {
    ...
    direction: "汉语言文学方向",
    concerns: "分数线、就业方向、校园生活",
    status: "持续观察",
  },
];

const currentLeadAdvice = [
  { label: "学生画像", content: "广东 / 物理类 / 585 分 / 电子信息方向" },
  { label: "核心关注", content: "就业去向、转专业政策、近三年录取位次" },
  { label: "意向判断", content: "多次追问专业就业与录取可能性，报考意愿较高。" },
  { label: "建议动作", content: "建议招生老师 24 小时内人工跟进。" },
  { label: "推荐材料", content: "电子信息类就业案例、专业培养方案、近三年录取位次说明。" },
];

function LeadModuleItem({ title, desc, index }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, amount: 0.45 }}
      transition={{ delay: 0.22 + index * 0.08, duration: 0.42, ease: [0.22, 1, 0.36, 1] }}
      className="grid gap-3 border-t border-slate-200/80 py-4 sm:grid-cols-[5.6rem_1fr]"
    >
      <h3 className="text-[16px] font-black tracking-[-0.035em] text-slate-950">{title}</h3>
      <p className="text-[14px] leading-6 text-slate-500">{desc}</p>
    </motion.div>
  );
}

function LeadWorkflowLine() {
  return (
    <div className="border-b border-slate-200/80 px-5 py-4">
      <div className="flex flex-wrap items-center gap-2 text-[12px] font-bold text-slate-500">
        {leadWorkflow.map((step, index) => (
          <React.Fragment key={step}>
            <span className="rounded-md bg-slate-50 px-2.5 py-1 text-slate-600 ring-1 ring-slate-200/80">{step}</span>
            {index < leadWorkflow.length - 1 && <span className="text-slate-300">→</span>}
          </React.Fragment>
        ))}
      </div>
    </div>
...
function LeadStudentRow({ student, index }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, amount: 0.5 }}
      transition={{ delay: 0.35 + index * 0.07, duration: 0.36, ease: [0.22, 1, 0.36, 1] }}
      className={`grid gap-3 border-b border-slate-100 px-5 py-3.5 sm:grid-cols-[5rem_1fr_auto] sm:items-center ${student.active ? "bg-sky-50/48" : "bg-white/0"}`}
    >
      <div>
        <p className="text-[15px] font-black tracking-[-0.03em] text-slate-950">{student.name}</p>
        <p className="mt-1 text-[11px] font-semibold text-slate-400">{student.profile}</p>
      </div>
      <div className="min-w-0">
        <p className="text-[14px] font-black text-slate-700">{student.direction}</p>
        <p className="mt-1 truncate text-[12px] leading-5 text-slate-500">关注：{student.concerns}</p>
      </div>
      <div className={`w-fit rounded-md border px-2.5 py-1 text-[12px] font-black ${student.active ? "border-sky-200 bg-white text-sky-700" : "border-slate-200 bg-white/70 text-slate-500"}`}>
        {student.status}
      </div>
    </motion.div>
  );
...
function LeadAdvicePanel() {
  return (
    <div className="px-5 py-4">
      <h4 className="text-[16px] font-black tracking-[-0.035em] text-slate-950">当前线索建议</h4>
      <div className="mt-3 grid gap-x-5 gap-y-2.5 sm:grid-cols-2">
        {currentLeadAdvice.map((item, index) => (
          <motion.div
            key={item.label}
            initial={{ opacity: 0, y: 8 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.6 }}
            transition={{ delay: 0.56 + index * 0.05, duration: 0.3 }}
            className={index === currentLeadAdvice.length - 1 ? "sm:col-span-2" : ""}
          >
            <p className="text-[11px] font-black text-slate-400">{item.label}</p>
            <p className="mt-0.5 text-[13px] leading-5 text-slate-600">{item.content}</p>
          </motion.div>
        ))}
      </div>
    </div>
...
function HighIntentWorkspace() {
  return (
    <motion.div
      ...
      className="relative overflow-hidden border border-white/80 bg-white/72 shadow-[0_28px_90px_rgba(14,165,233,0.12)] backdrop-blur-xl"
    >
      ...
      <div className="flex items-start justify-between gap-4 border-b border-slate-200/80 px-5 py-4">
        <div>
          <h3 className="text-[22px] font-black tracking-[-0.045em] text-slate-950">高意向生源工作台</h3>
          <p className="mt-1 text-[13px] leading-5 text-slate-500">从咨询记录中提取学生画像，筛选值得人工跟进的线索。</p>
        </div>
        <span className="shrink-0 rounded-md border border-sky-100 bg-sky-50 px-2.5 py-1 text-[12px] font-black text-sky-700">线索已筛选</span>
      </div>
      <LeadWorkflowLine />
      <div className="bg-white/54">
        {leadStudents.map((student, index) => (
          <LeadStudentRow key={student.name} student={student} index={index} />
        ))}
      </div>
      <LeadAdvicePanel />
    </motion.div>
...
function DashboardPage() {
  return (
    <section className="relative h-[100svh] overflow-hidden bg-[#f8fbff] text-slate-950">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_18%_20%,rgba(186,230,253,0.34),transparent_30%),radial-gradient(circle_at_86%_28%,rgba(125,211,252,0.24),transparent_34%),linear-gradient(180deg,#f8fbff_0%,#ffffff_42%,#edf8ff_100%)]" />
      <div className="pointer-events-none absolute left-[-14rem] top-[10%] h-[32rem] w-[32rem] rounded-full bg-sky-100/36 blur-3xl" />
      <div className="pointer-events-none absolute right-[-16rem] bottom-[4%] h-[34rem] w-[34rem] rounded-full bg-cyan-100/34 blur-3xl" />
      <div className="pointer-events-none absolute inset-x-0 top-0 h-24 bg-gradient-to-b from-[#f8fbff] via-[#f8fbff]/82 to-transparent" />
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-20 bg-gradient-to-b from-transparent via-[#f8fbff]/72 to-[#f8fbff]" />

      <div className="relative z-10 flex h-full items-center justify-center px-6 py-6">
        <div className="mx-auto grid w-full max-w-[86rem] scale-[0.90] transform-gpu items-center gap-9 lg:grid-cols-[0.86fr_1.14fr]">
          <motion.div
            variants={fadeUp}
            initial="hidden"
            whileInView="show"
            ...
            className="min-w-0"
          >
            <div className="mb-7 flex flex-wrap items-center gap-x-5 gap-y-1 text-sm font-semibold text-slate-500">
              <span className="text-[22px] font-black tracking-[-0.04em] text-slate-950">招生智脑</span>
              <span className="text-slate-300">|</span>
              <span>高校数字招生解决方案</span>
            </div>

            <p className="mb-4 text-[14px] font-black tracking-[0.18em] text-sky-600">生源线索识别</p>
            <h2 className="max-w-[42rem] text-[46px] font-black leading-[1.12] tracking-[-0.06em] text-slate-950 xl:text-[54px]">
              <span className="block whitespace-nowrap">从咨询数据中，</span>
              <span className="block whitespace-nowrap">识别值得跟进的学生</span>
            </h2>
            <p className="mt-5 max-w-[39rem] text-[16px] font-light leading-7 text-slate-500">
            ...
            <div className="mt-8 max-w-[40rem]">
              {leadModules.map((item, index) => (
                <LeadModuleItem key={item.title} {...item} index={index} />
              ))}
            </div>
          </motion.div>
